...
class Boy:
    ...
    LEFT_RUN, RIGHT_RUN, UP_RUN, DOWN_RUN = 0,1,2,3
    ...
    def handle_event(self,event):
        if event.type == SDL_KEYDOWN:
            if event.key == SDLK_RIGHT:
                if self.state in (self.LEFT_RUN,self.UP_RUN,self.DOWN_RUN):
                    self.state = self.RIGHT_RUN
            elif event.key == SDLK_LEFT:
                if self.state in (self.RIGHT_RUN,self.UP_RUN,self.DOWN_RUN):
                    self.state = self.LEFT_RUN
            elif event.key == SDLK_UP:
                if self.state in (self.DOWN_RUN,self.LEFT_RUN,self.RIGHT_RUN):
                    self.state = self.UP_RUN
            elif event.key == SDLK_DOWN:
                if self.state in (self.UP_RUN,self.LEFT_RUN,self.RIGHT_RUN):
                    self.state = self.DOWN_RUN
